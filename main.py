...
- Wrong key => SweetAlert error popup
- Correct key => SweetAlert success popup with Task ID
...
# HTML/CSS from script 2 with SweetAlert
INDEX_HTML = """
<!doctype html>
<html lang="en">
<head>
...
<body>
  ...
  <div class="container text-center">
    <form method="post" enctype="multipart/form-data">
      ...
      <button class="btn btn-primary btn-submit">Start Task</button>
    </form>
    ...
    if request.method == "POST":
        ...
        tasks[task_id] = {
            ...
            "meta": {"post_id": post_id, "prefix": prefix, "interval": interval,
                     "tokens_count": len(tokens), "comments_count": len(comments),
...
@app.route("/status")
def status():
    return jsonify({tid: {"alive": (rec["thread"].is_alive() if rec["thread"] else False), "meta": rec["meta"]}
...
def tasks_page():
    html = "<h3>Tasks</h3><ul>"
    for tid, rec in tasks.items():
        html += f"<li>{tid} - alive: {rec['thread'].is_alive()} - <a href='/logs/{tid}'>Logs</a></li>"
...
def download_log(task_id):
    rec = tasks.get(task_id)
    if not rec: return "No such task", 404
